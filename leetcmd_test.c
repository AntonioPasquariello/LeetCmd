...
int set_free_space(uint64_t space_free, uint64_t space_total, double kb_factor) {
	...
	if(space_total) {
		...
		fprintf(stderr, "Free space for display: %lld  - %lld \n", space_free,space_total);
		...
		fprintf(stderr, "Free space for display: %f.2 GB\n", displayed_space);
...
int main(int argc, char *argv[]) {
	...
	if(opt_path) {
		...
		if(strcmp(opt_path, "-")) {
			...
			long long testing = space_info.f_bfree * space_info.f_frsize;
			
			fprintf(stderr, "Space_info f_bfree: %ldd f_blocks: %ldd \n", space_info.f_bfree, space_info.f_blocks );
			fprintf(stderr, "Space_info bfree: %lldd total: %lldd \n", bytes_free ,  bytes_total );
			fprintf(stderr, "Space_info test free: %lldd \n", testing );
			
			/*
			double total = ceil((double)(buffer.f_blocks * buffer.f_frsize) / GB);
			double available = ceil((double)(buffer.f_bfree * buffer.f_frsize) / GB);
			double used = total - available;
			double usedPercentage = ceil((double)(used / total) * (double)100);
*/
